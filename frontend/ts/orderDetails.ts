...
function renderOrderDetails(response: any): void {
  ...
  if (response.order.voucher_code) {
    ...
    $("#order-voucher").text(
      `- ${formatCurrency(response.order.voucher_discount)}`
    );
  ...
  response.items.forEach(function (item: any) {
    $("#order-items").append(createOrderItemCard(item));
  });
}

function createOrderItemCard(item: any): JQuery<HTMLElement> {
...
function createInvoiceElement(orderResponse: any): HTMLElement {
...
function createInvoiceItemRow(item: any): JQuery<HTMLElement> {
...
function createInvoiceVoucher(order: any): JQuery<HTMLElement> {
