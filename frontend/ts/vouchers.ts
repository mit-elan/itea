...
$(document).ready(function () {
  ...
  $("#create-voucher-form").on("submit", function (e) {
    ...
    const value = parseFloat($("#voucher-value").val() as string);
    const validUntil = $("#voucher-valid-until").val() as string;
    ...
    $.ajax({
      ...
      success: function (vouchers: Voucher[]) {
        ...
        if (vouchers.length === 0) {
          $tableBody.append(
            `<tr><td colspan="4" class="text-center py-4">No vouchers found.</td></tr>`,
          );
          return;
        }
