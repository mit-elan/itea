...
const CATEGORY_NAME_TO_ID: { [key: string]: number } = {
  black: 1,
  green: 2,
  fruit: 3,
  herbal: 4,
...
$(document).ready(function () {
  ...
  function loadProducts(method: String, id?: number) {
    console.log("Mehtode:" + method + "ID:" + id);
    $.ajax({
      url:
        "/itea/backend/serviceHandler.php?handler=products&method=" +
        method +
        "&id=" +
        id,
      method: "GET",
  ...
  function renderProducts(products: Product[]) {
    ...
    products.forEach((product) => {
      ...
      const categoryName =
        CATEGORY_ID_TO_NAME[product.category_id] || "unknown";
